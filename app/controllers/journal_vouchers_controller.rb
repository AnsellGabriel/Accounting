class JournalVouchersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @journal_voucher.save
        format.html { redirect_to journal_voucher_url(@journal_voucher), notice: "Journal voucher was successfully created." }
  ...
  def update
    respond_to do |format|
      if @journal_voucher.update(journal_voucher_params)
        format.html { redirect_to journal_voucher_url(@journal_voucher), notice: "Journal voucher was successfully updated." }
  ...
  private
    ...
    def journal_voucher_params
      params.require(:journal_voucher).permit(:voucher, :date_voucher, :payee_id, :address, :description, :period_id, :branch_id)
    end
end
